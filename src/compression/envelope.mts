...
/**
 * Decodes the legacy node-storage envelope layout (v0) that did not include
 * an algorithm id and always used zstd.
 *
 * Legacy v0 layout:
 * - 4 bytes magic
 * - 4 bytes original size (UInt32LE)
 ...
 * @param {Buffer} payload - Envelope bytes to decode.
 * @param {Buffer} expectedMagic - Expected 4-byte format marker.
 * @param {CompressionAlgorithm} algorithm - Algorithm to assign to decoded payload.
 * @returns {CompressionResult | null} Parsed compression result, or null when payload does not match legacy format.
 */
export function deserializeLegacyCompressionEnvelopeV0(
  payload: Buffer,
  expectedMagic: Buffer,
  ...
  const originalSize = payload.readUInt32LE(4);
  const compressedSize = payload.readUInt32LE(8);
  const payloadStart = COMPRESSION_ENVELOPE_HEADER_SIZE;
  const payloadEnd = payloadStart + compressedSize;
