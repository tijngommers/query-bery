...
function isWsModule(value: unknown): value is WsModule {
    ...
    return typeof candidate.WebSocketServer === "function"
...
export class WsServer {
    ...
    private isClientMessage(value: unknown): value is ClientMessage {
        ...
        switch (value.type) {
            ...
            case "PartitionNodes":
                return Array.isArray(value.groups) && value.groups.every(
                    (group) => Array.isArray(group) && group.every((nodeId) => typeof nodeId === "string")
                );
            case "HealPartition":
            case "HealAllLinks":
                return true;
            ...
            case "AddServer":
                return this.hasString(value, "nodeId")
                    && this.hasString(value, "address")
                    && typeof value.asLearner === "boolean";
            default:
                return false;
        }
    }
